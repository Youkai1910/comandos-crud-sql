...
Funções de agregação realizam cálculos ou processos em registros de um resultados.
...
Retorna ao maior preço existente:
...
`GROUP BY` reune registros que possuem um determinado
valor em comum.
...
### Determinando a média de preço por fonecedor
...
Exemplo: montar somente os fornecedores que possuem pelo menos dois produtos cadastrados.
...
4. Mantem apenas fornecedores com pelo menos dois produtos
5. Ordena os grupos pelo preço médio

```sql
SELECT fornecedor_id, COUNT(*) AS total_produtos, ROUND (AVG(preco), 2) AS preco_medio
FROM produtos
WHERE quantidade > 0
